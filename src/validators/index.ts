...
export const isRegexMatched = (regex: RegExp) =>
	createValidator(value => {
		if (typeof value !== "string") return false
		return regex.test(value)
	})

export const isEqualTo = (value: any) => createValidator(val => val === value)
...
export const isGreaterThanOrEqualTo = (number: number) =>
	createValidator(value => {
		if (typeof value === "number") return value > number
		if (typeof value === "string" || Array.isArray(value)) {
			return value.length > number
...
export const isLesserThan = (number: number) =>
	createValidator(value => {
		if (typeof value === "number") return value < number
		if (typeof value === "string" || Array.isArray(value)) {
			return value.length < number
...
export const isLesserThanOrEqualTo = (number: number) =>
	createValidator(value => {
		if (typeof value === "number") return value <= number
		if (typeof value === "string" || Array.isArray(value)) {
			return value.length <= number
...
export const isOutOfRange = (min: number, max: number) =>
	createValidator(value => {
		if (typeof value === "number") return value < min && value > max
		if (typeof value === "string" || Array.isArray(value)) {
			return value.length < min && value.length > max
